...
chk("每个节日都有配置开关",
    all(f["key"] + "_test" in W.DEFAULT_CONFIG for f in W.FESTIVALS))
...
print("== 5. 同时只激活一个 ==")
cfg = {f["key"] + "_test": True for f in W.FESTIVALS}
f = W.active_festival(cfg, date(2026, 6, 1))
chk("全开时只返回一个（优先级最高的春节）", f is not None and f["key"] == "spring",
    f["key"] if f else None)
cfg2 = {"christmas_test": True, "halloween_test": True, "valentine_test": True}
chk("多个西方节日同开 → 取优先级高的圣诞",
    W.active_festival(cfg2, date(2026, 6, 1))["key"] == "christmas")
cfg3 = {f["key"] + "_test": False for f in W.FESTIVALS}
chk("全关 + 非节日 → None", W.active_festival(cfg3, date(2026, 6, 1)) is None)
chk("按日期自动命中（2026-09-24 中秋）",
    W.active_festival(cfg3, date(2026, 9, 24))["key"] == "midautumn",
    str(W.active_festival(cfg3, date(2026, 9, 24))))
chk("中秋当天(9-25)不再触发", W.active_festival(cfg3, date(2026, 9, 25)) is None)
chk("测试开关优先于日期",
    W.active_festival({"valentine_test": True}, date(2026, 9, 24))["key"] == "valentine")

print("== 6. 缓存一致 ==")
a = W.active_festival(cfg3, date(2026, 9, 24))
b = W.active_festival(cfg3, date(2026, 9, 24))
chk("同样入参结果一致", (a["key"] if a else None) == (b["key"] if b else None))
chk("缓存命中后仍正确", W.active_festival(cfg3, date(2026, 10, 1)) is None)
